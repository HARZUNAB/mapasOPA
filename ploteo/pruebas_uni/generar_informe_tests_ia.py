#!/usr/bin/env python3
"""Genera INFORME_DETALLADO_TESTS_IA.docx con el detalle completo de las pruebas
unitarias sugeridas por el asistente (IA) para la marcha blanca de NewPT.

Equivale al contenido del antiguo INFORME_TESTS_MARCHA_BLANCA.docx pero marcado
explícitamente como tests sugeridos por IA, para mantener la separación respecto
al informe de las pruebas escritas de forma guiada (M1-M5).
"""
...
SALIDA = os.path.join(DIR, "IA_pruebas", "INFORME_DETALLADO_TESTS_IA.docx")
...
def main():
    doc = Document()
    doc.add_heading("Informe de pruebas — Tests sugeridos por IA", 0)
    doc.add_paragraph(
        "Detalle de las 31 pruebas unitarias propuestas por el asistente (IA) y "
        "que integran la marcha blanca del proyecto NewPT (contexto sismotectónico "
        "de eventos sísmicos), como control de calidad previo a la compilación de "
        "binarios."
    )
    doc.add_paragraph(
        "Estas pruebas se mantienen SEPARADAS del informe de las pruebas escritas "
        "de forma guiada (M1-M5, documentadas en INFORME_DETALLADO_MIS_TESTS_M1_M5.docx)."
    )
    ...
    doc.add_paragraph(
        ...
        "nada. Este conjunto sugerido por IA cubre:"
    )
    for t in [
    ...
    doc.add_paragraph(
        ...
        "coordinado de la ventana 2D/3D)."
    )
    tabla(
        doc,
    ...
    nota(doc, f"Este informe documenta los 31 tests sugeridos por IA. En paralelo, "
              f"pruebas_uni/mis_pruebas/ contiene 41 tests adicionales (M1-M5) desarrollados "
              f"de forma guiada, documentados en INFORME_DETALLADO_MIS_TESTS_M1_M5.docx. "
              f"La suite completa suma 72 pruebas.")
