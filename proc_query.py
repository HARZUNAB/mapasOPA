...
# 2. PROCESAMIENTO ORIGINAL (Con corrección de orden Latitud/Longitud)
with open(sys.argv[2], 'w') as fh:
    for row in datos_ordenados:
        try:
            ...
            # CAMBIO AQUÍ: Se invirtió el orden de 'longitud' y 'latitud' para que coincida con lo solicitado
            fh.write("%s\t%.3f\t%.3f\t%.1f\t%.1f\t%s\t%s\t%s\n" % (
                fecha_fmt, latitud, longitud, prof, mag, tipo_mag, perc, ref 
            ))
