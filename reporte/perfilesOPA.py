...
#print("Se están iniciando los procesos para generar mapas de perfiles...")
...
sub_dir = sys.argv[3].split('/')[-1] # toma solo el nombre final
...
if not df_sensibles.empty:
    ax.scatter(df_sensibles['lon'], df_sensibles['lat'], marker='o', s=tamanio_sensibles, c='tomato', linewidth=.4, edgecolors='k', label="Percib.")
if not df_nosensibles.empty:
    ax.scatter(df_nosensibles['lon'], df_nosensibles['lat'], marker='o', s=tamanio_nosensibles, c='teal', linewidth=.4, edgecolors='k', label="No Percib.")
...
#print("="*60)
"""
# Abrir las imágenes generadas
img_planta = os.path.join(ruta_final, "Chileplanta_perfiles.png")
img_perfiles = os.path.join(ruta_final, "Perfiles.png")

webbrowser.open(os.path.abspath(img_planta))
webbrowser.open(os.path.abspath(img_perfiles))

print("Imágenes abiertas en el visor del sistema.")
"""
