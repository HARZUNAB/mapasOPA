...
def consultar_por_fecha_creacion(id_origen):
    ...
    # Selección de Query SQL según el tipo de ID recibido (LEFT JOIN unificado)
    ...
    try:
        ...
        # CONTROL ESTRICTO: Si no hay fila devuelta por la base de datos o el ID del evento es nulo,
        # significa que NO es el origen preferido actual del evento en SeisComP.
        if not ev or not ev['id_evento']:
            print(f"\n[X] ATENCIÓN: El ID {id_origen} no retornó resultados válidos como solución preferida.")
