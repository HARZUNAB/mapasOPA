...
for arg in "$@"; do
    case "$arg" in
        ...
        --probar|--marcha-blanca) SOLO_PROBAR=1 ;;
        --ayuda|-h)
            echo "USO: $0 [DIR_INSTALACION] [--solo-verificar] [--probar] [--ayuda]"
            exit 0
            ;;
        *) DIR_INSTALACION="$arg" ;;
    esac
...
# -----------------------------------------------------------------------------
# 1b. MARCHA BLANCA (--probar): crea un venv temporal, corre los tests y sale.
#     No instala ni toca nada en DIR_INSTALACION.
# -----------------------------------------------------------------------------
if [ "$SOLO_PROBAR" -eq 1 ]; then
    F_PRUEBAS="$FUENTES_DIR";   [ -d "$F_PRUEBAS" ]  || F_PRUEBAS="$ESCENARIO_DIR"
    D_PRUEBAS="$DATOS_DIR_SRC"; [ -d "$D_PRUEBAS" ] || D_PRUEBAS="$ESCENARIO_DIR"
    [ -d "$F_PRUEBAS/pruebas" ] || { echo "[ERROR] No existe $F_PRUEBAS/pruebas"; exit 1; }

    TMPD="$(mktemp -d /tmp/newpt_probar.XXXXXX)"
    trap 'rm -rf "$TMPD"' EXIT
    VENV_P="$TMPD/venv"

    echo ""
    echo "==> MARCHA BLANCA: creando venv temporal en $TMPD ..."
    "$PYTHON" -m venv "$VENV_P"
    "$VENV_P/bin/pip" install --quiet --upgrade pip
    echo "==> Instalando módulos con versiones FIJAS (puede tardar varios minutos)..."
    "$VENV_P/bin/pip" install "${PAQUETES_FIJOS[@]}"

    if ejecutar_pruebas "$VENV_P" "$D_PRUEBAS" "$F_PRUEBAS"; then
        echo ""
        echo "============================================================"
        echo "  MARCHA BLANCA SUPERADA: todos los tests pasaron."
        echo "============================================================"
        exit 0
    else
        echo ""
        echo "============================================================"
        echo "  MARCHA BLANCA FALLIDA: hay tests que no pasan."
        echo "  Corrige los casos listados arriba y reintenta:"
        echo "      $0 --probar"
        echo "============================================================"
        exit 1
    fi
...
[ -f "$FUENTES_DIR/newpt_servidor.py" ]   || { echo "[ERROR] Falta fuentes/newpt_servidor.py"; exit 1; }
...
# Mismas versiones fijadas que en --probar (ver PAQUETES_FIJOS al inicio).
"$VENV/bin/pip" install "${PAQUETES_FIJOS[@]}"
...
# -----------------------------------------------------------------------------
# 4b. MARCHA BLANCA: PRUEBAS UNITARIAS (control obligatorio previo a compilar)
# -----------------------------------------------------------------------------
if ! ejecutar_pruebas "$VENV" "$DATOS_DIR_SRC" "$FUENTES_DIR"; then
    echo ""
    echo "============================================================"
    echo "  INSTALACIÓN DETENIDA: la marcha blanca (tests) falló."
    echo ""
    echo "  La instalación NO continúa a la compilación de binarios."
    echo "  Corrige los casos listados arriba y vuelve a ejecutar:"
    echo "      $0 $DIR_INSTALACION"
    echo "  Para iterar solo sobre los tests:"
    echo "      $0 --probar"
    echo "============================================================"
    exit 1
fi
echo ""
echo "==> Marcha blanca superada: continuando con la compilación."
...
cp -r "$DIR_INSTALACION/.dev/fuentes/dist/newpt_servidor" "$DIR_INSTALACION/bin/"
...
echo "  bin/newpt_servidor/           -> daemon residente (mantiene la ventana)"
