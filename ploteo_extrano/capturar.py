...
#
def plotear_evento(fecha, lat, lon, prof, mag, event_id, texto_magnitud, bloquear=True):
    """
    ...
    Con bloquear=True (modo standalone, por defecto) la función llama a
    plt.show() y retorna cuando el operador cierra la ventana. Con
    bloquear=False (uso desde el daemon residente newpt_servidor) la ventana
    se dibuja y la función retorna de inmediato, dejando el bucle de eventos
    de Tk a cargo del daemon, que mantiene la misma ventana entre eventos.
    """
    # === ETAPA 1: Iniciando lectura ===
    mostrar_avance(1, 4, "Filtrando sismicidad histórica...")

    # Cuando este módulo se usa desde el daemon residente (newpt_servidor), una
    # misma ventana se REUTILIZA para varios eventos. Antes de dibujar el nuevo
    # se cierran las figuras 2D/3D del evento anterior (cierre coordinado) y se
    # reinician las banderas de estado, evitando acumular ventanas. Solo se
    # cierran figuras de eventos (no la de control del daemon, marcada con
    # _newpt_control). En modo standalone (proceso único) no hay figuras
    # previas, así que es inocuo.
    global _VENTANA_3D_ABIERTA, _FIGURA_3D_ACTIVA
    figuras_previas = [f for f in plt.get_fignums()
                       if not getattr(plt.figure(f), "_newpt_control", False)]
    if figuras_previas:
        try:
    ...
    plt.show() if bloquear else plt.draw()
    if bloquear:
